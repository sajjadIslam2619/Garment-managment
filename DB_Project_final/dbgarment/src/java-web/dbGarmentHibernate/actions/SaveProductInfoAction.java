...
import dbGarmentHibernate.forms.ProductInfoForm;
 ...
 * @struts.action path="/saveProductInfo"
 *                type="dbGarmentHibernate.actions.SaveProductInfoAction"
 *                name="productInfoForm"
 *                scope="request"
 *                input="/productinfo.jsp"
 * @struts.action-forward name="success" path="/initProductInfoSearch.do"
 *
 */
public class SaveProductInfoAction extends ServiceAction {
   ...
   public ActionForward doPerform(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      ...
      ProductInfoForm subform = (ProductInfoForm) form;
      String idProduct = subform.getIdProduct();
      if (idProduct != null && (idProduct.equalsIgnoreCase("null") || idProduct.length() < 1)) {
         idProduct = null;
         subform.setIdProduct(null);
      }
      dbGarmentHibernate.session.IProductInfoService facade = getProductInfoServiceService();
      ...
      try {
         ...
         if ("new".equals(subform.getMode())) {
            if (request.getParameter("submit") != null) {
               facade.addProductInfo(data);
            }
         }
         else {
            if (request.getParameter("submit") != null) {
               facade.saveProductInfo(data);
            }
            else if (request.getParameter(Constants.SUBMIT_NO_VALIDATION) != null) {
               facade.deleteProductInfo(data.getIdProduct());
            }
         }
      }
      catch (Throwable t) {
         log.error("SaveProductInfoAction :", t);
         throw new Exception("SaveProductInfoAction :", t);
      }
