...
/**
 * Struts 'Init' Action; prepares any information needed prior to displaying the 'productTypeForm'.
 *
 * @author  DB team  Finalist IT Group
 * @version $Revision: 1.4 $, $Date: 2006/04/27 06:56:25 $
 * @struts.action path="/initProductType"
 *                type="dbGarmentHibernate.actions.InitProductTypeAction"                 
 *                name="productTypeForm"
 *                scope="request"
 *                validate="false"
 * @struts.action-forward name="success" path="/producttype.jsp"
 *
 */
public class InitProductTypeAction extends ServiceAction {
   ...
   public ActionForward doPerform(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request,
      ...
      ProductTypeForm form = (ProductTypeForm) actionForm;
      HttpSession session  = request.getSession();
      String idProductType = request.getParameter("idProductType");

      if (idProductType != null && idProductType.equalsIgnoreCase("null")) {
         idProductType = null;
      }
      ...
      if (idProductType != null &&  true) { // hack to allow automatic generation of sources with JAG
         try {
            form.setMode("change");
            ProductType producttype = getProductTypeServiceService().getProductType(new java.lang.Integer(idProductType));
            PropertyUtilsExt.copyProperties(form, producttype);

         }
         catch (Throwable t) {
            servlet.log("InitProductTypeAction :", t);
            throw new ServletException("InitProductTypeAction :", t);
         }
      }
      else {
         form.setMode("new");
         PropertyUtilsExt.copyProperties(form, new ProductType());
      }
